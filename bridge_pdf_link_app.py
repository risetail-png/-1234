...
def _parse_photo_page_nums(text):
    ...
    アプローチ:
    1. 「写真番号」を含む行を対象に、N-M形式（損傷図参照メモ）を除いて数字を収集。
       「写真番号」直後の数字も確実に拾えるよう「写真番号」をスペースに置換してから抽出。
    2. 撮影日（YYYY.MM.DD）に結合した数字（例: 2016.06.218 の 8）を
       min〜max+1 の範囲で追加回収。
    """
    text = _normalize_text(text)
    ...
    for line in text.split('\n'):
        # N-M形式（損傷図参照メモ「写真番号1-1の接写」等）は行ごとスキップ
        if re.search(r'写真番号\s*\d+\s*[-－]\s*\d+', line):
            continue
        if '写真番号' not in line:
            continue
        line = re.sub(r'前回\s*[-－]?\s*\d*', '', line)   # 「前回-N」除去
        line = re.sub(r'写真番号', ' ', line)              # 「写真番号」→スペース
    ...
    base  = min(nums)
    upper = max(nums) + 1   # 撮影日に結合した数字を回収する余裕

    # ── Step2: 撮影日に結合した数字を追加回収 ──────────────────────────────
    work = text
    work = re.sub(r'(\d{4}[./]\d{2}[./]\d{2})(\d+)', r' \2 ', work)
    work = re.sub(r'\d{4}[./]\d{2}[./]\d{2}', '', work)
    work = re.sub(r'前回\s*[-－]?\s*\d*', '', work)
    work = re.sub(r'[-－]\s*\d+', '', work)
    work = re.sub(r'\d+\s*[-－]\s*\d+', '', work)

    for n in re.findall(r'\b(\d{1,3})\b', work):
        v = int(n)
        if base <= v <= upper:
            nums.add(v)
...
def run_process(input_path, output_path, dpi, log_cb, done_cb):
    ...
    try:
        ...
        # ── 対応関係を構築 ───────────────────────────────────────────────────
        # 基本方針: 径間番号の一致を最優先。
        # OCR写真番号で絞り込んだとき、同径間の全ページがカバーされなければ
        # 絞り込みを捨てて同径間の全ページをリンクする。
        diag_to_photo_pages = defaultdict(list)

        for didx, info in diagram_info.items():
            span_nums  = info['span_nums']   # {span: set of photo_num}  N-M形式
            plain_nums = info['plain_nums']  # set of photo_num  単独/OCR形式
            diag_span  = info['diag_span']

            # この損傷図ページが対象とする径間番号セットを決定
            spans_in_diag = set(span_nums.keys()) if span_nums else set()
            if diag_span is not None:
                spans_in_diag.add(diag_span)

            seen_pages = set()

            # 全径間の候補ページ数と、写真番号照合でマッチしたページ数を比較
            # 照合で一部しか取れない場合は同径間全ページをリンク
            all_nums = set()
            if span_nums:
                for nums in span_nums.values():
                    all_nums |= nums
            all_nums |= plain_nums

            for span in sorted(spans_in_diag):
                candidates = span_to_photo_pages.get(span, [])
                if not candidates:
                    continue

                if all_nums:
                    matched = [pp for pp in candidates
                               if set(photo_page_nums.get(pp, [])) & all_nums]
                    # マッチしたページが全候補の半分未満の場合は全ページリンク
                    targets = matched if len(matched) >= len(candidates) / 2 else candidates
                else:
                    targets = candidates
